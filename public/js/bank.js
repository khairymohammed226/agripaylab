...
function showBankMessage(text, type) {
 ...
 if (currentUser) {
  ...
  (async () => {
    try {
      const res = await fetch(`https://agripay.online/card/${currentUser._id}`);
...
showBankMessage("", "success");
  ...
  try {
    ...
    const check = await fetch(`https://agripay.online/transfer/check-card-password`, {
      method: "POST",
...
const transfer = await fetch(`https://agripay.online/transfer/bank-transfer`, {
  method: "POST",
  headers: { "Content-Type": "application/json" },
